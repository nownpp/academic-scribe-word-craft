...
export const generateWordDocument = async (
  ...
  try {
    ...
    const fileName = title ? `${title.substring(0, 50).replace(/[^\w\s-]/g, '')}.docx` : 'البحث_العلمي.docx';
...
const isMainTitle = (text: string, title: string): boolean => {
  return text === title || (text.length < 100 && title && text.includes(title.substring(0, 20)));
};
